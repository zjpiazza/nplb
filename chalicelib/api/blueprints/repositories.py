...
from ...services.github import GitHubService  # This might not be needed anymore since we're using SQS
from ...services.repository import RepositoryService  # This might not be needed anymore
...
def get_build_status(job_id):
    response = resources.sqs.get_queue_url(
        QueueName=job_id
    )
...
@blueprint.route('/{repo_id}', methods=['DELETE'])
def delete_repository(repo_id):
    response = resources.repository_table.delete_item(
        Key={'id': repo_id}
    )
